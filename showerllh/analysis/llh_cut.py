...
def getLLHhists(s, cut, binWidth):
    ...
    llh_bins = np.linspace(pf_ratio.min(), pf_ratio.max(), 200)
    llh_mids = (llh_bins[1:]+llh_bins[:-1]) / 2.
    e_start  = Ebins[0]
    newEbins = np.array([e_start])

    # Begin writing hists
    h = {}
    while e_start < Ebins[-1]:
        e_end = e_start + binWidth
        newEbins = np.append(newEbins, e_end)
        ...
        for comp in ['P','Fe']:
            c0 = s[comp] * cut * ecut
            h[e_range][comp] = np.histogram(pf_ratio[c0], bins=llh_bins)[0]
...
def makeLLHcut(s, cut, binWidth, p, plot=False):
    ...
    llh_mids = (llh_bins[1:]+llh_bins[:-1]) / 2.
    e_start = Ebins[0]
    newbins = np.array([e_start])
    pllh, fllh = np.array([]), np.array([])

    while e_start < Ebins[-1]:
        e_end = e_start + binWidth
        newbins = np.append(newbins, e_end)
        ecut = (r>=e_start) * (r<e_end)
        e_start += binWidth
        ...
        for true_comp in ['P','Fe']:
            ...
            h[true_comp] = np.histogram(pf_ratio[c0], bins=llh_bins)[0]
        ...
        pllh = np.append(pllh, llh_mids[(Ntot_p * p_cut).argmax()])
        fllh = np.append(fllh, llh_mids[(Ntot_f * f_cut).argmax()])
...
def getLLHcut(llhMax=10):
    ...
    cutFile = '%s/llh_cuts.npy' % my.llh_resource
    llhDict = np.load(cutFile)
...
if __name__ == "__main__":
    ...
    my.setupShowerLLH(verbose=False)
    config = 'IT73'
    outFile = '%s/llh_cuts.npy' % my.llh_resource
