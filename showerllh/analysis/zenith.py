...
def effCheck(s, cut, st=45, comp='joint', ndiv=1, zbins=3):
    ...
    for key in keys:
        ...
        nplt = len(eff)/ndiv
        eff_joint, rel_joint, err_joint, en_joint = np.zeros((4, nplt))
        for j in range(nplt):
            start = ndiv*j
            end = ndiv*(j+1)
...
def lnA(q, cut, st=45, zbins=4, mc=False):
    ...
    if not mc:
        rList = ['p','h','o','f']
        r = np.log10(q['ML_energy'])
    else:
        rList = ['P','H','O','F']
        r = np.log10(q['MC_energy'])
    ...
    a = np.sum([N[e] for e in rList], axis=0)
