...
# E.g. for centered at the origin xy0=[0,0]
...
if __name__ == "__main__":
    ...
    if not args.nhex:
        ...
        print('args.nhex = {}'.format(args.nhex))
    ...
    print('zip = {}'.format(zip(args.steps, args.steps[1:])))
    if not all(i > j for i, j in zip(args.steps, args.steps[1:])):
        raise ValueError('Grid steps must be in decreasing size')
    if len(args.nhex) != len(args.steps) - 1:
        raise ValueError(
            'Incorrect number of arguments for either nhex or steps')
