...
def IsInHAWCMask(alpha, delta):
    """ Return true if coordinates are within the IC59 mask region 
    """
    return delta >= (-26*degree) and delta <= (64*degree)
...
def SetupColorBar(fig, title=None, ticks=None, coord="C", flip="astro"):
    ...
    for ax in fig.get_axes():
        if type(ax) is H.projaxes.HpxMollweideAxes:
            ...
            if coord == "C":
                #ax.annotate("0h", xy=(1.8, 0.625), size="xx-large")
                #ax.annotate("24h", xy=(-1.95, 0.625), size="xx-large")
                if flip == "astro":
                   #ax.annotate("0$^\circ$", xy=(1.8, 0.625), size="x-xx-large")
                   #ax.annotate("360$^\circ$", xy=(-1.95, 0.625), size="x-xx-large")
                   ax.annotate("0$^\circ$", xy=(1.8, -0.75), size="xx-large")
                   ax.annotate("360$^\circ$", xy=(-1.95, -0.75), size="xx-large")
                else:
                   ax.annotate("360$^\circ$", xy=(1.8, 0.625), size="xx-large")
                   ax.annotate("0$^\circ$", xy=(-1.95, 0.625), size="xx-large")
                #ax.annotate("0$^\circ$", xy=(1.8, -0.625))
                #ax.annotate("360$^\circ$", xy=(-1.95, -0.625))
            else:
                if flip == "astro":
                   ax.annotate("-180$^\circ$", xy=(1.65, 0.625), size="xx-large")
                   ax.annotate("180$^\circ$", xy=(-1.9, 0.625), size="xx-large")
                else:
                   ax.annotate("180$^\circ$", xy=(1.65, 0.625), size="xx-large")
                   ax.annotate("-180$^\circ$", xy=(-1.9, 0.625), size="xx-large")
...
def cmap_discretize(cmap, N):
    ...
    cmap = matplotlib.colors.LinearSegmentedColormap('colormap',cdict,1024)
    cmap.set_under('w')
    return cmap
...
def getR(x):
  if x < 0.32 * 0.78125:
    return 0
  if x > 0.32 * 0.78125 and x < 0.32 * 1.78125:
...
def getB(x):
  ...
  if x < 0.92:
   ...
   # return x < 0.25 ? 4 * x : x < 0.42 ? 1 : x < 0.92 ? -2*x + 1.84 : x / 0.08 - 11.5 

def SetupBVRYColormap():
    cdict = { 'red':   ((0.0,  getR(0.0), getR(0.0)),
                    (0.34, getR(0.34), getR(0.34)),
                    (0.61, getR(0.61), getR(0.61)),
                    (0.84, getR(0.84), getR(0.84)),
                    (1.0, getR(1.0), getR(1.0))),

          'green': ((0.0,  getG(0.0), getG(0.0)),
                    (0.34, getG(0.34), getG(0.34)),
                    (0.61, getG(0.61), getG(0.61)),
                    (0.84, getG(0.84), getG(0.84)),
                    (1.0, getG(1.0), getG(1.0))),
          
          'blue': ((0.0,  getB(0.0), getB(0.0)),
                    (0.34, getB(0.34), getB(0.34)),
                    (0.61, getB(0.61), getB(0.61)),
                    (0.84, getB(0.84), getB(0.84)),
                    (1.0, getB(1.0), getB(1.0)))
        }
    return LinearSegmentedColormap('BVRY', cdict)

def SetupYRVBColormap():
    cdict = { 'red':   ((0.0,  getR(1.0), getR(1.0)),
                    (0.14, getR(0.84), getR(0.84)),
                    (0.41, getR(0.61), getR(0.61)),
                    (0.74, getR(0.34), getR(0.34)),
                    (1.0, getR(0.0), getR(0.0))),

          'green': ((0.0,  getG(1.0), getG(1.0)),
                    (0.14, getG(0.84), getG(0.84)),
                    (0.41, getG(0.61), getG(0.61)),
                    (0.74, getG(0.34), getG(0.34)),
                    (1.0, getG(0.0), getG(0.0))),

          'blue': ((0.0,  getB(1.0), getB(1.0)),
                    (0.14, getB(0.84), getB(0.84)),
                    (0.41, getB(0.61), getB(0.61)),
                    (0.74, getB(0.34), getB(0.34)),
                    (1.0, getB(0.0), getB(0.0)))
        }

    return LinearSegmentedColormap('YRVB', cdict)
...
if __name__ == "__main__":
    ...
    parser.add_option("-a", "--avgsubtract", action="store_true", dest="avg",
                      default=False, help="Subtract average from plot")
    ...
    parser.add_option("-C", "--colormap", dest="colormap", default=None,
                      help="Colormap")
    ...
    if options.mask != "X":
        ...
        for i in range(0, npix):
            ...
            if options.mask == "H":
               if not IsInHAWCMask(alpha, delta):
                  if options.whiteBkg:
                     map[i] = inf
                  else:
                     map[i] = H.UNSEEN
    ...
    if options.colormap:
        colormap = pycolor.getColormap(options.colormap)
