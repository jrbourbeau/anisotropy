...
def load_sim(config):
    ...
    prefix = '/net/user/fmcnally/itanisotropy/showerLLH/%s_sim' % config
    MC_prefix = '/net/user/fmcnally/ShowerLLH/%s_sim' % config
    infile = '%s/SimPlot.npy' % prefix
    t0 = time.time()
    s = np.load(infile)
    s = s.item()
    ...
    s['cuts']['out'] = (abs(np.log10(s['ML_energy'])-np.log10(s['MC_energy'])) > 0.5)
    ...
    s['MC'] = np.load('%s/%s_MC.npy' % (MC_prefix, config))
    s['MC'] = s['MC'].item()
    ...
    for erange in ['low', 'med', 'high']:
        s['MC']['joint'][erange] = np.zeros(shape, dtype=int)
        for i in range(3,5):
            for j in range(i):
                k = str(i) + str(j)
                s['MC']['joint'][erange+'_'+k] = np.zeros(shape, dtype=int)
