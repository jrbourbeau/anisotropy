...
def counts(d, r, cut):
    ...
    #ax1.set_title('Counts using '+cutName+' cut')
    ax1.set_title('Counts using cut')
    ...
    for e in eList:
        N[e]   = Nfinder(r, cut*d[e], w=w)
        Err[e] = Efinder(r, cut*d[e], w=w)

    # Plot reconstructions
    for e in eList + ['All']:
        pnt = colorDict[e] + '.'
        ax1.errorbar(Emids, N[e], yerr=Err[e], fmt=pnt, label=e)
...
def flux(d, s, niter, spec=0, smooth=True):
    ...
    for i in range(niter):
        ...
        #Rel['All'] = sqrt(1/Nun['All'] + relerr**2 + unrel[i]**2)
        Rel['All'] = sqrt(1/Nun['All'] + relerr**2)
    ...
    #IT26_mids = log10(IT26_data['E'])
    #IT26_flux = asarray(IT26_data['dN/dE'])
