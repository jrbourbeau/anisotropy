...
def eres_ztest(s, cut, minE=6.0, zbins=5, out=False):
    ...
    theta0, theta1 = 0, arccos(0.8)
    Zbins = array([theta0])
    sigma = 2*pi*(cos(theta0) - cos(theta1)) / zbins
    while len(Zbins) < zbins+1:
        theta  = arccos(cos(theta0) - sigma/(2*pi))
        Zbins  = append(Zbins, theta)
        theta0 = theta

    x, med, min, max = zeros((4,zbins))
