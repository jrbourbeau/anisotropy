...
def getKnots(pts, k, n):
    ...
    num = around((end - start) / step) + 1
    knots = linspace(start, end, num=num)
    knots += step/10.
    return knots
...
def spline(s, p0, nk=2, npoly=3, plot=False):
    ...
    # Weight bins
    wts = ones(p0.shape)
    x = log10(s['MC_energy'][s['cuts']['llh']])
    y = log10(s['ML_energy'][s['cuts']['llh']])
    h, xedges, yedges = histogram2d(y, x, bins=(Ebins, Ebins[st:]))
    sigma = sqrt(var(h, axis=0))
    ntrue = sum(h, axis=0)
...
def spltest(s, bin=40, nk=2, npoly=3, clean=1):
    ...
    #cut = s['cuts']['llh']
    #x = log10(s['MC_energy'][cut])
    #y = log10(s['ML_energy'][cut])
    #h, xedges, yedges = histogram2d(y, x, bins=(Ebins, Ebins[st:]))
    #sigma = sqrt(var(h, axis=0))[bin]
