...
def maker(outFile, outFile_i3, fileList):
    ...
    keys += ['ShowerLLHParams', 'slc_count']
    keys += ['MCPrimary', 'pulse_x', 'pulse_y', 'pulse_time', 'pulse_charges']
    keys += ['hlc_count', 'd_pulse', 'd_str', 'pulse_z', 'tres', 'hits', 'qtot']
    keys += ['MaskedOfflinePulses', 'FilterMask', 'om_row']
    t0 = time.time()
    ...
    def prescale(frame):
        filterMask = frame['FilterMask']
        filterMask.remove('FilterMinBias_11')
        if not frame['FilterMask']['FilterMinBias_11'].prescale_passed:
            return True
        else:
            for filter in filterMask:
                if filterMask[filter].condition_passed:
                    return True
            return False
    
    #tray.AddModule(prescale, 'prescale')
