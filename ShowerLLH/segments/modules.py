...
################################################################################
# Modules necessary for ShowerLLH cuts and running the program in general
################################################################################

from icecube import icetray, dataio, toprec, simclasses
from icecube import dataclasses as dc
import sys

##============================================================================
## Functions used when running ShowerLLH
...
# Remove PoleMuonLlhFit from IceTop Frame
def rmPoleMuon(frame):
    if frame['I3EventHeader'].sub_event_stream == 'ice_top':
        if frame.Has('PoleMuonLlhFit'):
            frame.Delete('PoleMuonLlhFit')
            frame.Delete('PoleMuonLlhFitParams')
...
def moveSmall(frame):
    ...
    if frame['NStations'] < 5.:
        ...
        if frame.Has('LaputopFixed'):
            frame.Delete('LaputopFixed')
            frame.Delete('LaputopFixedParams')
            frame['LaputopFixed'] = frame['LaputopSmallFixed']
            frame['LaputopFixedParams'] = frame['LaputopSmallFixedParams']
...
class LoudestStationOnEdge(icetray.I3Module):
    ...
    def __init__(self, context):
        ...
        self.AddParameter('output','What is the name of the output Bool in the frame',0)
        self.AddOutBox('OutBox')
