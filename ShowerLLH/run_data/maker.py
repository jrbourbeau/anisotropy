...
if __name__ == "__main__":
    ...
    p.add_argument('--held', dest='held', action='store_true',
            default=False,
            help='Run over jobs that took over 12 hours')
    args = p.parse_args()
    ...
    if config == 'IT81-II':
        ...
        if args.job:
            ...
            files = []
            gcdFiles = []
            for i in glob.glob('/home/zgriffith/npx4/npx4-logs/*.log'):
                if 'Policy violation' in open(i).read():
                    jobs.append(i[36:-4])
            for job in jobs:
                f      = open('/home/zgriffith/npx4/npx4-execs/npx4-'+job+'.sh')
                files += [i for i in f.readlines()[-2].split(' ') if i[-3:] == 'bz2' and not 'Geo' in i]
                f      = open('/home/zgriffith/npx4/npx4-execs/npx4-'+job+'.sh')
                gcdFiles += [i for i in f.readlines()[-2].split(' ') if i[-2:] == 'gz' and 'GCD' in i] 
        else:
